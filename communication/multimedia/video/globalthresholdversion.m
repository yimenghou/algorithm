% lab 3 task 4,5,6,7 This one use a global threshold! 
...
type = 'packet'; % 'noisy' or 'packet' or 'none'
t = 60; % how many errors per packet
crate = 0.5; % compression rate (This is fixed in this project)
lostpacketrate = 0.03; % lost packet rate
...
% global threshold
shaped = abs(reshape(dctcoef, 1, N1 * N2)); % remove small DCT coefficients
sorted = sort(shaped);
th = sorted(round(crate * N1 * N2));
dctcoef(abs(dctcoef) < th) = 0;

signal = reshape(dctcoef,[],1);
%% quantization 
level = 255;
...
invsignal = buffer(afterdequantized,256);
compressedctinv = invsignal;
Invi = zeros(N1, N2); 
count = 1;
for i = 1: M1
    for j = 1:M2
...
figure; imshow(Invi); title('Transmitted image');
