...
class saveWP(object):
	...
	def __init__(self):
		...
		# init pub,sub
		self.position_sub = rospy.Subscriber("/gnss_pose", PoseStamped, callback=self.positionCallback)
		self.twist_sub = rospy.Subscriber("/husky_velocity_controller/odom", Odometry, callback=self.twistCallback)
		self.orientation_sub = rospy.Subscriber("/imu/data", Imu, callback=self.orientationCallback)
	...
	# callback functions
	def positionCallback(self, position_msg):
		# get position from lla2pose
		self.waypoint_current.pose.pose.position = position_msg.pose.position

	def twistCallback(self, twist_msg):
		# get twist from odometry
		self.waypoint_current.twist.twist = twist_msg.twist.twist

	def orientationCallback(self, orientation_msg):
		# get orientation from lla2pose or imu
		self.waypoint_current.pose.pose.orientation = orientation_msg.orientation
