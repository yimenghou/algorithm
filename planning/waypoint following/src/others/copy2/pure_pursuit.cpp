...
void PurePursuit::getNextWaypoint()
{
  ...
  waypoint_number_end = next_waypoint_number_+lookahead_waypoint_ < path_size? next_waypoint_number_+lookahead_waypoint_ :path_size;

  for (int i = next_waypoint_number_; i < waypoint_number_end; i++) //next_waypoint_number_+lookahead_waypoint_
  {

    // if (i < next_waypoint_number_)
    // {
    //   continue;
    // }
    ...
    if (temp_dist > lookahead_distance_ && temp_dist < shortest_dist)
    {
...
bool PurePursuit::canGetCurvature(double *output_kappa)
{
  ...
  ROS_INFO("next waypoint index: %i", next_waypoint_number_);
  ROS_INFO("total number of waypoints: %i", static_cast<int>(current_waypoints_.size()));
